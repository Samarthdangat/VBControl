...
def getdistance(frame, landmark_ist):
...
while cap.isOpened():
    ...
    left_landmark_list, right_landmark_list = getlandmarks(frame, processed, draw, mpHands)
    ...
    if left_landmark_list:
        left_distance = getdistance(frame, left_landmark_list)
    ...
    if right_landmark_list:
        right_distance = getdistance(frame, right_landmark_list)
        vol = np.interp(right_distance, [50, 220], [-65.25, 0])
        volume.SetMasterVolumeLevel(vol, None)
        print(right_landmark_list)
